...
SELECT COUNT(*) INTO C 
FROM USER_SEQUENCES 
  WHERE SEQUENCE_NAME = 'SEQ_CONDICIONES_ID_CONDICION'; 
  IF (C > 0) THEN 
    EXECUTE IMMEDIATE 'DROP SEQUENCE "SEQ_CONDICIONES_ID_CONDICION"'; 
END IF; 
END;
/
...
SELECT COUNT(*) INTO C 
FROM USER_SEQUENCES 
  WHERE SEQUENCE_NAME = 'SEQ_TIPO_ID_TIPO'; 
  IF (C > 0) THEN 
    EXECUTE IMMEDIATE 'DROP SEQUENCE "SEQ_TIPO_ID_TIPO"'; 
END IF; 
END;
/
...
(
	...
	"USUARIO" VARCHAR2(128) NULL 	-- Usuario que se relaciona con el que tiene en la tabla dba_users
)
;
...
CREATE SEQUENCE "SEQ_SOLICITUDES_ID_SOLICITUD" 
	...
	NOORDER
;
CREATE SEQUENCE "SEQ_CONDICIONES_ID_CONDICION" 
	INCREMENT BY 1 
	START WITH 1 
	NOMAXVALUE 
	MINVALUE  1 
...
CREATE SEQUENCE "SEQ_TIPO_ID_TIPO" 
	INCREMENT BY 1 
	START WITH 1 
	NOMAXVALUE 
	MINVALUE  1 
...
CREATE OR REPLACE TRIGGER "TRG_CONDICIONES_ID_CONDICION" 
	BEFORE INSERT 
	ON "CONDICIONES" 
	FOR EACH ROW 
	BEGIN 
		SELECT TO_CHAR("SEQ_CONDICIONES_ID_CONDICION".NEXTVAL,'XXX') 
...
CREATE OR REPLACE TRIGGER "TRG_TIPO_ID_TIPO" 
	BEFORE INSERT 
	ON "TIPO" 
	FOR EACH ROW 
	BEGIN 
		SELECT TO_CHAR("SEQ_TIPO_ID_TIPO".NEXTVAL,'XXX') 
		INTO :NEW."ID_TIPO" 
		FROM DUAL; 
	END;
